...
from models.metric.MLP import MLP, LR
...
def train_mlp(
    ...
    visualization.plot_jasons_lineplot(None, train_loss_list, 'updates', 'training loss', f"{cfg.exp_id} n_train_c={cfg.samples_per_class} max_val_acc={max(val_acc_list):.3f}", f"plots/{cfg.exp_id}/train_loss.png")
    visualization.plot_jasons_lineplot(None, val_acc_list, 'updates', 'validation accuracy', f"{cfg.exp_id} n_train_c={cfg.samples_per_class} max_val_acc={max(val_acc_list):.3f}", f"plots/{cfg.exp_id}/val_acc{max(val_acc_list):.3f}.png")
    # torch.save(model.state_dict(), 'triplet/models/baseline_covid_weights.pt')

if __name__ == '__main__':
    from utils import common, configuration
    cfg_json_list = [
        "config/mlp/vanilla/mlp_jd_1_shot.json",
    ]

    for cfg_json in cfg_json_list:
        cfg = configuration.mlp_config.from_json(cfg_json)
